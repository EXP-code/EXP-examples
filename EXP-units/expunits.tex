...
\geometry{margin=1in}
\begin{document}
\title{Unit conversion to a system with \(G=1\) for \textsc{exp}}
...
As a simple example, my usual recommendation to Gadget users is to
keep their familiar length scale, velocity scale and time scale, and
multiple the particle masses by \(G_{\rm old}\).  Using these
relations, we have \(s_L=s_V=s_T=1\) and \(s_M = 1/G_{\rm old}\) (see
eq. \ref{eq:scaledunits}).
